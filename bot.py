...
def server_settings_config() -> dict:
    ...
    settings = load_config().get("server_settings", {})
    return {**defaults, **settings}


def update_server_settings(values: dict) -> dict:
    return update_config({"server_settings": values})["server_settings"]
...
def react_role_config() -> dict:
    ...
    return {**defaults, **load_config().get("react_role", {})}


def update_react_role_config(values: dict) -> dict:
    return update_config({"react_role": values})["react_role"]
...
HELP_SECTIONS = {
    ...
    "server_settings": (
        ...
        "**.joinrole <add/remove> <@role/id/nom>** - ajoute ou retire un rôle automatique à l'arrivée\n"
        "**.joinrole list** - affiche les rôles donnés à l'arrivée\n"
        "**.joinchannel <#salon/id/del>** - configure le salon des arrivées\n"
        "**.joinmessage <texte>** - configure le message d'arrivée (`{member}`, `{server}`)\n"
        "**.joinembed <on/off>** - active ou désactive l'embed d'arrivée\n"
        "**.joincolor <#hex>** - configure la couleur de l'embed d'arrivée\n"
        "**.joinembed test** - teste l'embed d'arrivée",
    ),
    "react_role": (
        "React Role",
        "**.reactrole** - ouvre la configuration interactive du rôle réaction",
    ),
    "embeds": (
        "Embeds",
...
class TicketConfigSelect(discord.ui.Select):
    def __init__(self) -> None:
        options = [
            ...
            discord.SelectOption(label="Modifier la couleur d'embed", value="embed_color", emoji="🎨"),
...
def join_settings_embed(guild: discord.Guild) -> discord.Embed:
    ...
    role_text = "\n".join(role.mention for role in roles if role) or "Aucun"
    ...
    embed.add_field(name="Channel de leave", value="Non configuré", inline=True)
    embed.add_field(name="Join embed", value=bool_icon(settings.get("join_embed_enabled", False)), inline=True)
    embed.add_field(name="Leave embed", value="❌", inline=True)
    embed.add_field(name="Couleur embed", value=settings.get("join_embed_color") or "#ff5ec7", inline=True)
    embed.add_field(name="Rôles membre", value=role_text, inline=True)
    embed.add_field(name="Ghost join", value="Aucun", inline=True)
...
async def ask_server_setting_value(interaction: discord.Interaction, field: str) -> None:
    ...
    prompts = {
        ...
        "add_role": "Envoie le rôle à ajouter avec @role, son ID, ou son nom.",
        "remove_role": "Envoie le rôle à retirer avec @role, son ID, ou son nom.",
    }
    await interaction.response.send_message(f"{prompts[field]}\nTape `cancel` pour annuler.")
    prompt_message = await interaction.original_response()
...
class ServerSettingsSelect(discord.ui.Select):
    def __init__(self) -> None:
        settings = server_settings_config()
        options = [
            discord.SelectOption(label="Modifier le salon de join", value="join_channel", emoji="📌"),
            discord.SelectOption(label="Modifier le message de join", value="join_message", emoji="💬"),
            discord.SelectOption(label="Modifier la couleur d'embed", value="join_color", emoji="🎨"),
            discord.SelectOption(label="Activer/Désactiver l'embed de join", value="toggle_join_embed", emoji="✅" if settings.get("join_embed_enabled") else "❌"),
            discord.SelectOption(label="Ajouter un rôle membre", value="add_role", emoji="➕"),
            discord.SelectOption(label="Retirer un rôle membre", value="remove_role", emoji="➖"),
            discord.SelectOption(label="Tester l'embed de join", value="test_join", emoji="🧪"),
        ]
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if choice == "test_join":
            settings = server_settings_config()
            message = format_join_message(settings.get("join_message") or server_settings_config()["join_message"], interaction.user)
            embed = discord.Embed(description=message, color=parse_hex_color(settings.get("join_embed_color") or "#ff5ec7"))
            await interaction.response.send_message(embed=embed)
            return
...
class ReactRoleSelect(discord.ui.Select):
    def __init__(self) -> None:
        options = [
            discord.SelectOption(label="Choisir le salon", value="channel", emoji="📌"),
            discord.SelectOption(label="Choisir le rôle attribué", value="role", emoji="🎭"),
            discord.SelectOption(label="Modifier la réaction", value="emoji", emoji="✅"),
            discord.SelectOption(label="Modifier le titre", value="title", emoji="✏️"),
            discord.SelectOption(label="Modifier le message", value="description", emoji="💬"),
            discord.SelectOption(label="Modifier la couleur d'embed", value="color", emoji="🎨"),
            discord.SelectOption(label="Envoyer le panneau", value="send", emoji="📨"),
            discord.SelectOption(label="Configurer accès serveur", value="access", emoji="🔒"),
        ]
...
@bot.group(name="reactrole", invoke_without_command=True)
@is_admin()
async def reactrole(ctx: commands.Context) -> None:
    await ctx.send(embed=react_role_embed(ctx.guild), view=ReactRoleSettingsView())
...
async def joinrole(ctx: commands.Context, action: str, *, role_value: str | None = None) -> None:
    ...
    if action == "list":
        roles = [ctx.guild.get_role(int(role_id)) for role_id in role_ids if str(role_id).isdigit()]
        text = "\n".join(role.mention for role in roles if role) or "Aucun rôle join configuré."
        await ctx.reply(text)
        return

    if action not in {"add", "remove"}:
        await ctx.reply("Utilise `.joinrole <add/remove> <rôle>` ou `.joinrole list`.")
        return
    if not role_value:
        await ctx.reply("Rôle manquant.")
        return
...
@bot.command(name="joinchannel")
@is_admin()
async def joinchannel(ctx: commands.Context, *, channel_value: str) -> None:
    if channel_value.lower() in {"del", "delete", "remove", "off"}:
        update_server_settings({"join_channel_id": None})
        await ctx.reply("Salon d'arrivée retiré.")
        return
    ...
    if not channel:
        ...
        return
    update_server_settings({"join_channel_id": str(channel.id)})
    await ctx.reply(f"Salon d'arrivée configuré: {channel.mention}.")


@bot.command(name="joinmessage")
@is_admin()
async def joinmessage(ctx: commands.Context, *, text: str) -> None:
    update_server_settings({"join_message": text})
    await ctx.reply("Message d'arrivée configuré.")


@bot.command(name="joincolor")
@is_admin()
async def joincolor(ctx: commands.Context, color: str) -> None:
    cleaned = color.strip()
    if not re.fullmatch(r"#?[0-9a-fA-F]{6}", cleaned):
        await ctx.reply("Couleur invalide. Exemple: `.joincolor #ff5ec7`.")
        return
    update_server_settings({"join_embed_color": "#" + cleaned.removeprefix("#")})
    await ctx.reply("Couleur de l'embed d'arrivée configurée.")


@bot.command(name="joinembed")
@is_admin()
async def joinembed(ctx: commands.Context, action: str) -> None:
    action = action.lower()
    if action in {"on", "off"}:
        update_server_settings({"join_embed_enabled": action == "on"})
        await ctx.reply(f"Embed d'arrivée {'activé' if action == 'on' else 'désactivé'}.")
        return
    if action == "test":
        settings = server_settings_config()
        message = format_join_message(settings.get("join_message") or server_settings_config()["join_message"], ctx.author)
        embed = discord.Embed(
            description=message,
            color=parse_hex_color(settings.get("join_embed_color") or "#ff5ec7"),
        )
        await ctx.send(embed=embed)
        return
    await ctx.reply("Utilise `.joinembed <on/off>` ou `.joinembed test`.")
...
async def update_command_permission(ctx: commands.Context, command_name: str, role_value: str, add: bool) -> None:
    ...
    await ctx.reply(f"{role.mention} {action} pour `.{command_name}`.")
...
async def createrole(ctx: commands.Context, *, name: str) -> None:
    ...
    try:
        ...
        return
    await ctx.reply(f"Rôle créé: {role.mention}.")
...
async def createchannel(ctx: commands.Context, *, name: str) -> None:
    ...
    try:
        ...
        return
    await ctx.reply(f"Salon créé: {channel.mention}.")
...
async def addrole(ctx: commands.Context, user_value: str, *, role_value: str) -> None:
    ...
    await ctx.reply("1 rôle ajouté à 1 membre.")
...
async def delrole(ctx: commands.Context, user_value: str, *, role_value: str) -> None:
    ...
    await ctx.reply("1 rôle retiré à 1 membre.")
...
async def massiverole(ctx: commands.Context, action: str, *, role_value: str) -> None:
    ...
    await status.edit(content=f"Massiverole terminé: {added} ajoutés, {skipped} ignorés, {failed} erreurs.")
